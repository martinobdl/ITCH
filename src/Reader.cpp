...
Message Reader::createMessage(void){
    ...
    switch(key){
        ...
        uint64_t timeStamp, orderId, oldOrderId, newOrderId, level1, level2, level3, matchNumber, matchId, size64, pairedShares, imbalanceShares;
        uint16_t locateCode, trackingNumb;
        uint32_t size, price, execSize, cancSize, newSize, newPrice,numberOfSharesInALot, ETPLevarage, timeIPO, priceIPO, auctionCollarPrice, upperAuctionCollarPrice, lowerAuctionCollarPrice, fairPrice, nearPrice, referencePrice;
        char auctionCollarExtension, direction, eventCode, marketCategory, financialStatus, roundLotsOnly, issueClassification, subType[3], autenticity, shortIndicator, ipoFlag, indicatorLULD, flagETP, inverseETPFlag, tradingState, reserved, reason[5], regSHO, mpidIdentifier[5], primaryMarketMaker, marketMakerMode, makerParticipantState, breachedLevel, qualifierIPO, marketCode, haltAction, mpid[5], printable, crossType, imbalanceDirection, priceVariationIndicator;
